...
const LandfillMap = () => {
  ...
  async function currentAuthenticatedUser() {
    try {
      ...
      setUserFullName(`${result.given_name} ${result.family_name}`);
    } catch (err) {
      console.log(err);
      return null;
    }
    ...
    .filter(function (well) {
      if (selectedPriorities.length == 0) {
        ...
        if (devices[well.gasWellsDevicesId]?.Services?.items.length > 0) {
          if (
            devices[well.gasWellsDevicesId]?.Services?.items.some((serv) =>
              selectedPriorities.includes(serv.priority)
            )
          ) {
            return well;
          }
  ...
  useEffect(() => {
    ...
    const fetchServiceTypes = async () => {
      try {
        const serviceTypeData = await client.graphql({
          query: listServiceTypes,
        });
        // document.title = landfill.name;
        const allServiceTypes = serviceTypeData.data.listServiceTypes.items.map(
          (servType) => ({
            value: servType.id,
            label: servType.name,
            ...servType,
  ...
  const fetchDevices = async () => {
    try {
      ...
      let newDeviceArray = deviceData.data.devicesByLandfillsID.items.reduce(
        (acc, obj) => {
          acc[obj.id] = obj;
          return acc;
        },
      ...
      if (updatedServices[service.devicesID]) {
        switch (action) {
          case "create":
            ...
            break;
          case "update":
            updatedServices[service.devicesID].Services.items = updatedServices[
              service.devicesID
            ].Services.items.map((s) => (s.id === service.id ? service : s));
            break;
          case "delete":
            updatedServices[service.devicesID].Services.items = updatedServices[
              service.devicesID
            ].Services.items.filter((s) => s.id !== service.id);
            break;
          default:
            break;
        }
  ...
  const ServiceList = ({ selectedGasWell, isComplete }) => {
    ...
    if (
      !selectedGasWell ||
      !devices[selectedGasWell.gasWellsDevicesId].Services
    ) {
    ...
    const allServices = devices[
      selectedGasWell.gasWellsDevicesId
    ]?.Services.items
    ...
    return (
      <div>
        {allServices.length > 0 ? (
            ...
            {allServices.map((service) => (
              <Flex
                ...
                onClick={() => {
                  ...
                  handleOpeningModal({
                    modalName: "serviceRequest",
                    modalTitle: "Edit Service Request",
                  });
                }}
              >
                <Badge
                ...
                >
                  {service.priority == "Medium" ? "Med" : service.priority}
                </Badge>
                <div>
                  <Text style={{ lineHeight: "1" }}>{service.title}</Text>
                  <Text color="dimmed" size="xs">
                    {service.userId ? `Added by ${service.userId} ` : ``}at{" "}
                    {moment(service.createdAt).format("l h:mm a")}
                  </Text>
                </div>
              </Flex>
          ...
          <p style={{ color: "var(--apis-gray-700)", fontSize: "0.85rem" }}>
            No {status} services found.
          </p>
        )}
      </div>
    );
  ...
  const renderModals = () => {
    switch (activeContent) {
      case "installation":
        return (
          <InstallationModal
            onSubmit={handleAddGasWell}
            landfillsID={id}
            gasWell={selectedGasWell}
          />
        );
      case "addGasWell":
        return <AddGasWell onSubmit={handleAddGasWell} landfillsID={id} />;
      case "serviceRequest":
        return (
            ...
            <div>
              ...
              <Group justify="flex-end">
                <Button
                  onClick={() => {
                    setSelectedDevice(
                      devices[selectedGasWell.gasWellsDevicesId]
                    );
                    setSelectedService(null);
                    handleOpeningModal({
                      modalName: "serviceRequest",
                      modalTitle: "Add Service Request",
                    });
                  }}
                >
                  Add Service Request
              ...
              <div style={{ margin: "1rem 0" }}>
                <Accordion multiple onChange={setOpenAccordion}>
                  ...
                  >
                    <Accordion.Control
                      style={{ borderBottom: "1px solid var(--apis-gray-200)" }}
                    >
                      To Do
                    </Accordion.Control>
                    <Accordion.Panel style={{ paddingTop: "0.5rem" }}>
                      <ServiceList
                        selectedGasWell={selectedGasWell}
                        isComplete={false}
                      />
                    </Accordion.Panel>
                  </Accordion.Item>
